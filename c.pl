...
sub lexer {
	...
	while($line ne '') {
		$line =~ s/^\s//;
		if($line =~ s/^(\[)//) {
			$rec->read('open_bracket',$1);
		}
		if($line =~ s/^(\])//) {
			$rec->read('close_bracket',$1);
		}
		if($line =~ s/^(,)//) {
			$rec->read('comma',$1);
		}
...
my $grammar = Marpa::XS::Grammar->new({ start => 'translation_unit',
                                        ...
                                        rules   => [
	                                    { lhs => 'translation_unit', rhs => [qw/external_decl/], action => 'list'},
	                                ...
	                                { lhs => 'direct_declarator', rhs => [qw/direct_declarator lbracket const_exp rbracket/], },
	                                ...
	                                { lhs => 'const', rhs => [qw/int_const/], },
	                                { lhs => 'const', rhs => [qw/char_const/], },
	                                { lhs => 'const', rhs => [qw/float_const/], },
	                                { lhs => 'const', rhs => [qw/enumeration_const/], },
	                                  ]});
